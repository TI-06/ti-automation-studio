...
const serviceDataSource = readIfExists('../src/data/services.ts');
...
describe('SEO Phase 1', () => {
  it('トップページが業務自動化の検索意図をtitleとH1で明示する', () => {
    ...
    expect(homeSource).toContain('Excel・GAS・Pythonで業務自動化');
    expect(homeSource).toContain('手作業を、現場で使える仕組みに変える');
  });

  it('既存ツールの修正ニーズ向けにVBAとGASの専用サービスを持つ', () => {
    expect(serviceDataSource).toContain("slug: 'vba-repair'");
    expect(serviceDataSource).toContain("slug: 'gas-repair'");
    expect(serviceDataSource).toContain('VBA修正');
    expect(serviceDataSource).toContain('GAS修正');
  });
  ...
  it('課題解決ページが関連記事ではなくサービス・実績・相談へつながる', () => {
